...
stylecheck: $(TEST_PATH)/compile_commands.json
	...
	@clang-tidy -p=$(TEST_PATH) -quiet -checks=*,-google-build-using-namespace,-fuchsia-default-arguments,-llvm-header-guard,-performance-unnecessary-value-param,-google-global-names-in-headers,-cppcoreguidelines-pro-bounds-constant-array-index,-cppcoreguidelines-pro-bounds-pointer-arithmetic,-cppcoreguidelines-pro-bounds-array-to-pointer-decay,-hicpp-no-array-decay,-readability-non-const-parameter -header-filter=.* -export-fixes=test/style.yaml $(IMPLEMS) $(HEADERS) $(DRIVER)
